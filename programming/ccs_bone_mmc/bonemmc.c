...
int mmchs_init()
{
	...
	set32(MMCHS0_REG_BASE + MMCHS_SD_SYSCONFIG,MMCHS_SD_SYSCONFIG_SOFTRESET,MMCHS_SD_SYSCONFIG_SOFTRESET);

	/* read sysstatus to know it's done */
	while (! (read32(MMCHS0_REG_BASE + MMCHS_SD_SYSSTATUS) & MMCHS_SD_SYSSTATUS_RESETDONE)){
		counter ++;
	}
	 ...
	 */
	set32(MMCHS0_REG_BASE + MMCHS_SD_CAPA,MMCHS_SD_CAPA_VS_MASK,MMCHS_SD_CAPA_VS18 | MMCHS_SD_CAPA_VS30);

	/*
	 * wake-up configuration
	 */
	set32(MMCHS0_REG_BASE + MMCHS_SD_SYSCONFIG,
	...
	set32(MMCHS0_REG_BASE + MMCHS_SD_HCTL,MMCHS_SD_HCTL_IWE, MMCHS_SD_HCTL_IWE_EN);
	...
	set32(MMCHS0_REG_BASE + MMCHS_SD_CON,MMCHS_SD_CON_DW8,MMCHS_SD_CON_DW8_1BIT);
	set32(MMCHS0_REG_BASE +MMCHS_SD_HCTL,MMCHS_SD_HCTL_DTW,MMCHS_SD_HCTL_DTW_1BIT);


	/* Configure card voltage  */
	set32(MMCHS0_REG_BASE + MMCHS_SD_HCTL,
			MMCHS_SD_HCTL_SDVS
			,
	...
	set32(MMCHS0_REG_BASE + MMCHS_SD_HCTL,MMCHS_SD_HCTL_SDBP ,MMCHS_SD_HCTL_SDBP_ON);

	while (  (read32(MMCHS0_REG_BASE + MMCHS_SD_HCTL) &  MMCHS_SD_HCTL_SDBP) != MMCHS_SD_HCTL_SDBP_ON){
		counter ++;
	}

	/* Enable internal clock and clock to the card*/
	set32(MMCHS0_REG_BASE + MMCHS_SD_SYSCTL,MMCHS_SD_SYSCTL_ICE,  MMCHS_SD_SYSCTL_ICE_EN);

	//@TODO Fix external clock enable , this one is very slow
	set32(MMCHS0_REG_BASE + MMCHS_SD_SYSCTL,MMCHS_SD_SYSCTL_CLKD, (0x3ff << 6));
	set32(MMCHS0_REG_BASE + MMCHS_SD_SYSCTL,MMCHS_SD_SYSCTL_CEN, MMCHS_SD_SYSCTL_CEN_EN);
	while(  (read32(MMCHS0_REG_BASE + MMCHS_SD_SYSCTL) & MMCHS_SD_SYSCTL_ICS) != MMCHS_SD_SYSCTL_ICS_STABLE);
	...
	set32(MMCHS0_REG_BASE + MMCHS_SD_IE,MMCHS_SD_IE_CC_ENABLE,MMCHS_SD_IE_CC_ENABLE_ENABLE);
	/* enable transfer complete interrupt */
	set32(MMCHS0_REG_BASE + MMCHS_SD_IE,MMCHS_SD_IE_TC_ENABLE,MMCHS_SD_IE_TC_ENABLE_ENABLE);
	 ...
	 */
	set32(MMCHS0_REG_BASE + MMCHS_SD_CON,MMCHS_SD_CON_INIT,MMCHS_SD_CON_INIT_INIT);
	write32(MMCHS0_REG_BASE + MMCHS_SD_CMD, 0x00); /* command 0 , type other commands , not response etc) */

	while( (read32(MMCHS0_REG_BASE + MMCHS_SD_STAT) & MMCHS_SD_STAT_CC) != MMCHS_SD_STAT_CC_RAISED){
		if (read32(MMCHS0_REG_BASE + MMCHS_SD_STAT) & 0x8000){
			printf("%s, error stat  %x\n", __FUNCTION__, read32(MMCHS0_REG_BASE + MMCHS_SD_STAT));
			return 1;
		}
		counter ++;
	}
	...
	set32(MMCHS0_REG_BASE + MMCHS_SD_STAT,MMCHS_SD_IE_CC_ENABLE,MMCHS_SD_IE_CC_ENABLE_ENABLE);

	/*
	 * Set Set SD_CON[1] INIT bit to 0x0 to end the initialization sequence
	 */
	set32(MMCHS0_REG_BASE + MMCHS_SD_CON,MMCHS_SD_CON_INIT,MMCHS_SD_CON_INIT_NOINIT);
	return 0;
}
...
int card_identification()
{
	...
	if (send_cmd(MMCHS_SD_CMD_CMD8 | MMCHS_SD_CMD_RSP_TYPE_48B,MMCHS_SD_ARG_CMD8_VHS  | MMCHS_SD_ARG_CMD8_CHECK_PATTERN)){
...
int card_query_voltage_and_type(struct sd_card * card)
{
	...
	if( send_cmd(SD_SEND_OP_COND | MMCHS_SD_CMD_RSP_TYPE_48B,(0x3F << 15) | (0x1 << 30))){
		return 1;
	}
	...
	while(1){//@TODO wait for max 1
		...
		/* Send ADMD41 */
		/* 0x1 << 30 == send HCS (Host capacity support) and get OCR register */
		if (send_cmd(SD_SEND_OP_COND | MMCHS_SD_CMD_RSP_TYPE_48B,(0x1FF << 15) | (0x1 << 30))){
			return 1;
		}
...
int card_csd(struct sd_card *card)
{
	...
	if (send_cmd(MMCHS_SD_CMD_CMD9 | MMCHS_SD_CMD_RSP_TYPE_136B, (card->rca << 16))) {
		return 1;
	}
...
int select_card(struct sd_card *card){
	/* select card */
	if (send_cmd(MMCHS_SD_CMD_CMD7 | MMCHS_SD_CMD_RSP_TYPE_48B_BUSY, (card->rca << 16))) {
			return 1;
	}
}
...
int write_single_block(struct sd_card *card, uint32_t blknr, unsigned char * buf)
{
	uint32_t count;
	uint32_t value;
